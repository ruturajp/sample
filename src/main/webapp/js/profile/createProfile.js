	...
	$scope.jobCodeSl = function(){
	 ...
	 $scope.uploadFileIntoDB = function (files) {
		...
		$scope.upload = function (files) {
			if(files[0].name.includes(".pdf") || files[0].name.includes(".doc") || files[0].name.includes(".docs") || files[0].name.includes(".docx")){
				$scope.fileError = true;
	...
	$scope.getDesignation = function() {  
		$scope.profiledesignations=[];
 		angular.forEach($scope.designations,function(obj){
 			var commSkills = _.intersection($scope.sk.primarySkills,obj.skills);
			if (commSkills.length > 0) {
				if(obj.minExpYear <= $scope.candidate.expYear && obj.maxExpYear >= $scope.candidate.expYear)
				{
