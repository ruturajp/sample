app.controller('interviewFeedbackCtrl',['$scope', '$http','$q', '$window','jobCodeService1', '$timeout', '$rootScope','$log','$state', '$location','profileService', 
                                        function($scope, $http, $q, $window, jobCodeService1, $timeout, $rootScope, $log, $state, $location,profileService) {
	...
	$scope.submit = function(){
		...
		profileService.addProfilesStatus($scope.emailId,$scope.interviewFeedback1.status)
		.then(function(data){
			alert(data);
	...
	$scope.setfeedbackData = function(roundName){
		...
		if(round.interviewSchedule.roundName == roundName || round.interviewSchedule.roundName == "Hr Round")	{
